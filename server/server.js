...
const CORS_WHITELIST = (
  process.env.CORS_WHITELIST || "https://example.com"
).split(",");
...
async function ensureDriveClient() {
  ...
  try {
    ...
    console.error("Drive client error", e);
    return null;
  }
}
    ...
    try {
      ...
      if (!drive || !DRIVE_ROOT_FOLDER_ID) {
        const localDir = path.resolve(
          process.cwd(),
          "uploads",
          `${dateSlug}_${clientName.replace(/\s+/g, "")}`,
        );
      ...
      // DRIVE mode
      const parent = await driveCreateFolder(
        drive,
        `${dateSlug}_${clientName}`,
...
app.use((req, res, next) => {
  ...
  // A basic Content Security Policy (CSP). This should be configured more specifically for production.
  res.setHeader(
    "Content-Security-Policy",
    "default-src 'self'; script-src 'self' 'unsafe-inline' https://plausible.io https://www.google.com https://www.gstatic.com https://browser.sentry-cdn.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://www.keystonenotarygroup.com; font-src 'self'; connect-src 'self'; frame-src 'self' https://www.google.com;",
  );
  next();
});
...
function pruneMemory() {
  ...
  for (const [k, v] of memory.sessions)
    if (v.exp < now) memory.sessions.delete(k);
}
...
async function issueToken(email) {
  ...
  if (redis) {
    await redis.set(`admintoken:${token}`, email, {
      EX: Math.floor(ADMIN_TOKEN_TTL_MS / 1000),
    });
  } else {
...
async function setSession(res, email) {
  ...
  if (redis) {
    await redis.set(`adminsession:${id}`, email, {
      EX: Math.floor(ADMIN_SESSION_TTL_MS / 1000),
    });
  } else {
    const exp = Date.now() + ADMIN_SESSION_TTL_MS;
    memory.sessions.set(id, { email, exp });
  }
  res.cookie("ks_admin", id, {
    httpOnly: true,
    sameSite: "lax",
...
app.post("/api/admin/request-magic-link", async (req, res) => {
  ...
  const url =
    (process.env.ADMIN_URL_BASE || "http://localhost:5173") +
    "/admin?token=" +
    token;
...
const suspicious = [];
function logSuspicious(entry) {
  suspicious.push({ ...entry, at: new Date().toISOString() });
}
app.get("/api/admin/suspicious", (req, res) =>
  res.json({ count: suspicious.length, items: suspicious.slice(-100) }),
);
...
function getSheetsClient() {
  ...
  try {
    ...
    console.error("Sheets JSON parse error", e);
    return null;
  }
}
  ...
  try {
    ...
    if (sheets && process.env.SHEETS_SPREADSHEET_ID) {
      const r = await sheets.spreadsheets.values.get({
        ...
        range: process.env.SHEETS_TESTIMONIALS_RANGE || "Testimonials!A:B",
      });
...
app.get("/api/cms/services", async (req, res) => {
  try {
    ...
    if (sheets && process.env.SHEETS_SPREADSHEET_ID) {
      const r = await sheets.spreadsheets.values.get({
        spreadsheetId: process.env.SHEETS_SPREADSHEET_ID,
        range: process.env.SHEETS_SERVICES_RANGE || "Services!A:B",
      });
  ...
  return res.json({
    ...
    data: [
      {
        title: "Loan Signings",
        body: "Purchase, refinance, HELOC, reverse—error‑free, lender‑friendly packages.",
      },
      {
        title: "General Notary Work",
        body: "POAs, affidavits, deeds, titles, I‑9s, and more—mobile to you.",
      },
      {
        title: "After‑Hours & Rush",
        body: "Evenings/weekends on request with punctual arrival windows.",
      },
      {
        title: "Business On‑Site",
        body: "Title, escrow, law offices, hospitals, senior communities—white‑glove service.",
      },
    ],
  });
});
...
app.get("/api/geocode", async (req, res) => {
  ...
  if (!process.env.GEOCODE_PROVIDER) {
    return res.json({
      ok: true,
      lat: 40.5795 + (Math.random() - 0.5) * 0.1,
      lon: -75.3407 + (Math.random() - 0.5) * 0.1,
      label: q + " (demo)",
    });
  }
  try {
    if (process.env.GEOCODE_PROVIDER === "nominatim") {
      const r = await fetch(
        ...
        { headers: { "User-Agent": "KeystoneNotary/1.0" } },
      );
    ...
    // Google Geocoding
    if (
      process.env.GEOCODE_PROVIDER === "google" &&
      ...
      return res.json({
        ...
        lat: loc.lat,
        lon: loc.lng,
        label: j.results[0].formatted_address,
      });
    }
...
app.get("/api/route", async (req, res) => {
  ...
  if (!process.env.ROUTING_PROVIDER) {
    ...
    const a1 = 40.5795,
      b1 = -75.3407;
    const dLat = toRad(lat - a1),
      dLon = toRad(lon - b1);
    const h =
      Math.sin(dLat / 2) ** 2 +
      Math.cos(toRad(a1)) * Math.cos(toRad(lat)) * Math.sin(dLon / 2) ** 2;
  ...
  if (RECAPTCHA_SECRET) {
    try {
      ...
      if (!json.success || (json.score && json.score < 0.5)) {
        logSuspicious({ name, email, score: json.score });
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        (process.env.SHEETS_CONTACTS_RANGE ||
          process.env.SHEETS_TESTIMONIALS_RANGE)
      ) {
    ...
    if (
      ...
      try {
        await sgMail.send({
          ...
          from: process.env.EMAIL_FROM || "no-reply@example.com",
          templateId: SENDGRID_TEMPLATE_OWNER,
          dynamicTemplateData: { name, email, phone, service, message },
        });
        if (email)
          await sgMail.send({
            to: email,
            from: process.env.EMAIL_FROM || "no-reply@example.com",
            templateId: SENDGRID_TEMPLATE_CLIENT,
            dynamicTemplateData: { name, service },
          });
      } catch (err) {
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    if (!OPENAI_API_KEY) {
      return res.json({
        reply:
          "Hi! This is the Keystone Notary demo assistant. I can answer general questions about mobile notarization and booking. For specific documents or legal guidance, call (267) 309‑9000 or email info@keystonenotarygroup.com.",
      });
    }
...
Phone: (267) 309-9000. Email: info@keystonenotarygroup.com. Avoid legal advice; suggest contacting us for specifics.`;
    ...
    if (!response.ok) {
      ...
      return res
        .status(500)
        .json({ error: "Upstream error", detail: t.slice(0, 500) });
    }
